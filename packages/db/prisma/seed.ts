...
const DEMO_PASSWORD_HASH = 'scrypt$oryn-demo-salt-v1$3bc2550329d0b9dabfccf5e4a28054706dca2dcc960661cf0d72a1abc23bced7';

async function main() {
  const organization = await prisma.organization.upsert({ where: { id: 'org_demo' }, update: {}, create: { id: 'org_demo', name: 'ORYN Demo Company' } });
  const admin = await prisma.user.upsert({ where: { id: 'usr_demo_admin' }, update: { passwordHash: DEMO_PASSWORD_HASH }, create: { id: 'usr_demo_admin', organizationId: organization.id, name: 'ORYN Admin', email: 'admin@oryn.local', role: UserRole.ADMIN, passwordHash: DEMO_PASSWORD_HASH } });
  const sales = await prisma.user.upsert({ where: { id: 'usr_demo_sales' }, update: { passwordHash: DEMO_PASSWORD_HASH }, create: { id: 'usr_demo_sales', organizationId: organization.id, name: 'Andi Pratama', email: 'sales@oryn.local', role: UserRole.SALES, passwordHash: DEMO_PASSWORD_HASH } });
  ...
  const customers = [['cus_budi','Budi Santoso','+628123456789','HOT_LEAD',91],['cus_nina','Nina Marlina','+628234567890','ACTIVE',78],['cus_andi','Andi Pratama','+628345678901','NEW',62]] as const;
  for (const [id, name, phone, status, leadScore] of customers) {
    ...
    if (!await prisma.message.findFirst({ where: { conversationId: conversation.id } })) await prisma.message.create({ data: { conversationId: conversation.id, direction: 'INBOUND', content: name === 'Budi Santoso' ? 'Kak, saya tertarik dengan paket premium. Berapa harganya?' : 'Halo, saya ingin informasi produk.', senderType: 'customer' } });
  }
  ...
  await prisma.auditLog.create({ data: { organizationId: organization.id, actorId: admin.id, action: 'DEMO_SEED', entityType: 'Organization', entityId: organization.id, metadata: { version: '0.4.0' } } });
